...
-- 숫자를 문자로 TO_CHAR(값, 형식(자릿수))
-- 형식에서 사용하는 '9'는 실제 숫자가 9가 아니라 자릿수를 표현하기 위한 기호입니다.
SELECT TO_CHAR(20000,'99999') FROM dual;

-- 주어진 자릿수에서 숫자를 모두 표기할 수 없어서 
-- #으로 표기됩니다.
...
SELECT '$3,300' + 2000 FROM dual;-- 에러(자동 형 변환 불가능)
SELECT TO_NUMBER('$3,300', '$9,999') + 2000 FROM dual; --(정수로 이루어지지 않은 수 연산)
...
/*
문제 1.
현재일자를 기준으로 employees 테이블의 입사일자(hire_date)를 참조해서 근속년수가 17년 이상인
...
-- FROM -> WHERE 순서로 진행되기 때문에 WHERR 근속년수 >= 17은 불가능함
-- 즉 WHERE 절에서는 별칭은 사용할 수 없습니다. (SELECT 보다 WHERE 절이 먼저 조회됨)
