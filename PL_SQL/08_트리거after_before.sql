...
/*
AFTER 트리거 - INSERT, UPDATE, DELETE 작업 이후에 동작하는
트리거를 의미합니다.
BEFORE 트리거 - INSERT, UPDATE, DELETE 작업 이전에 동작하는
트리거를 의미합니다.

:OLD = 참조 전 열의 값 (INSERT: 입력 전 자료, UPDATE: 수정 전 자료,
DELETE: 삭제될 값)
...
트리거 자체를 트랜잭션의 일부로 처리하기 때문에 COMMIT 이나 ROLLBACK을 포함할 수 없습니다. 
...
BEGIN
    ...
    IF v_product_total <= 0 THEN
        RAISE zero_total_exception;-- 예외 발생 코드??(= THROW?)
    ...
    -- 두번째 매개값: 에러 메세지
            RAISE_APPLICATION_ERROR(-20001,'주문하신 수량보다 재고가 적은 관계로 주문이 불가능 합니다.');
...
-- 트리거 내에서 예외가 발생하면 수행 중인 INSERT 작업은 중단되며
-- ROLLBACK이 진행됩니다.
